...
and writes sharded JSON ranges to public/ip2l6/<b0>/<b1>/<b2>/<b3>.json where b0..b3 are the top 32 bits of ip_from.
...
// Shards map: b0 -> Map(b1 -> Map(b2 -> Map(b3 -> Array(records))))
...
async function main() {
  ...
  for await (const line of rl) {
    ...
    const b3 = Number((s >> 96n) & 0xffn)
    if (!shards.has(b0)) shards.set(b0, new Map())
    const m1 = shards.get(b0)
    if (!m1.has(b1)) m1.set(b1, new Map())
    const m2 = m1.get(b1)
    if (!m2.has(b2)) m2.set(b2, new Map())
    const m3 = m2.get(b2)
    if (!m3.has(b3)) m3.set(b3, [])
    m3.get(b3).push({ s: s.toString(), e: e.toString(), c: country || null, r: region || null, ci: city || null })
  }
  ...
  for (const [b0, map1] of shards.entries()) {
    ...
    for (const [b1, map2] of map1.entries()) {
      ...
      for (const [b2, map3] of map2.entries()) {
        const dir2 = path.join(dir1, String(b2))
        fs.mkdirSync(dir2, { recursive: true })
        for (const [b3, arr] of map3.entries()) {
          arr.sort((a, b) => (BigInt(a.s) < BigInt(b.s) ? -1 : 1))
          const outPath = path.join(dir2, `${b3}.json`)
  ...
  console.log('Done. Place public/ip2l6/<b0>/<b1>/<b2>/<b3>.json on your host (Pages will serve them).')
}
